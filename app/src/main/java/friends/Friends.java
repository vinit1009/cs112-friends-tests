...
public class Friends {
	...
	public static ArrayList<String> shortestChain(Graph g, String p1, String p2) {
		...
		while(!unprocessed.isEmpty()) {
			...
			for(Friend i = current.first; i!=null; i = i.next) {
				...
				if(predecessors[g.map.get(friend.name)] == -1) predecessors[g.map.get(friend.name)] = g.map.get(current.name);
				if(friend.equals(end)) {
					...
					while(true) {
						...
						if(currName.equals(start.name)) break;
					}
					for(int j=chain.size()-2; j>=0; j--)chain.add(chain.remove(j));
					return chain;
	...
	public static ArrayList<ArrayList<String>> cliques(Graph g, String school) {
		...
		if(start.equals("")) return cliques;
		...
		while(!unprocessed.isEmpty()) {
			...
			for(Friend i = g.members[g.map.get(current)].first; i!=null; i = i.next) {
				Person friend = g.members[i.fnum];
				if(visited.contains(friend.name)) continue;
				if(friend!=null) {
				...
				if(friend.school!=null && friend.school.equals(school)) unprocessed.push(friend.name);
			}
	...
	private static ArrayList<String> cliquesHelper(Graph g, String school, ArrayList<String> visited, Person start){
		...
		while(!unprocessed.isEmpty()) {
			...
			for(Friend i=g.members[g.map.get(current)].first; i!=null; i = i.next) {
				Person friend = g.members[i.fnum];
				if(friend.school == null || !friend.school.equals(school) || visited.contains(friend.name)) continue;
	...
	public static ArrayList<String> connectors(Graph g) {
		...
		while(start<g.members.length){
			if(!visited.contains(g.members[start])) connectorsHelper(g, connectors, dfsValue, backValue, visited, start, true, dfsValues, backValues, backedAlready); start++;
		}
		return connectors;
		
	}
	private static void connectorsHelper(Graph g, ArrayList<String> connectors, int dfsValue, int backValue, ArrayList<Person> visited, int start, boolean isStart, HashMap<String, Integer> dfsValues, HashMap<String, Integer> backValues, ArrayList<String> backedAlready){
		...
		while(friend != null){
			...
			if(!visited.contains(pFriend)){
				connectorsHelper(g, connectors, dfsValue, backValue, visited, friend.fnum, false, dfsValues, backValues, backedAlready);
				if(dfsValues.get(current.name) > backValues.get(pFriend.name)){
					int minBack = Math.min(backValues.get(current.name), backValues.get(pFriend.name));
